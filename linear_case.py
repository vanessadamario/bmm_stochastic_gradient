...
def main():
    ...
    d = 30  # input space dimension (variables)
    train_dim = 0.5
    k = int(n * train_dim)
    ...
    w_12 = np.random.randn(d, d)
    w_32 = np.random.randn(1, d)
    b_1 = np.zeros(d,)
    b_2 = np.zeros_like(b_1)

    print("real function")
    predicted_matrix = np.dot(w_32, w_12)
    print(predicted_matrix.sum(axis=0))
    ...
    W1 = tf.Variable(1e-4*tf.ones([w_12.shape[0], w_12.shape[1]]))
    W2 = tf.Variable(1e-4*tf.ones([w_32.shape[0] ,w_32.shape[1]]))
    ...
    print("prediction")
    predicted_matrix = np.dot(curr_W2, curr_W1)
    print(predicted_matrix.sum(axis=0))
